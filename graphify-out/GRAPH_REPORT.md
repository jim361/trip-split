...
2026-09-15 · 기준 `dev` / `e6e706191c82fc6027331a958a50aff165de9874` 위 현재 작업 트리.
서버 구현·검증은 완료됐고 Flutter format/analyze/test/APK·실제 Android integration은 SDK 부재로 미검증이다. 현재 상태의 commit·push는 사용자가 승인했다.
...
- `watchChecklist` --shares_data_with--> `Firestore checklistitems`  [EXTRACTED]
  frontend/lib/data/firebase/firestore_trip_repositories.dart → backend/firestore.rules
...
Cohesion: 0.07
Nodes (70): parseReceipt, receiptImageMaxBytes, validateReceiptImage(), checkProvider(), demoPlaces, HTTPS Google Maps query 또는 q 링크, googleLinkQuery(), parsePlaceLink (+62 more)
...
Cohesion: 0.15
Nodes (40): allocationsField(), asRecord(), booleanField(), createFirestoreTripRepositories(), dataFrom(), epochMillisField(), errorCodeOf(), firebaseErrorMap (+32 more)
...
Cohesion: 0.11
Nodes (22): createDefaultServices(), DataSource, PlatformServicesContext, AuthErrorListener, AuthService, AuthStateListener, AuthUser, FirebaseClient (+14 more)
...
Cohesion: 0.10
Nodes (23): App(), AuthContext, AuthContextValue, AuthProvider(), AuthStatus, useAuth(), PlatformServicesProvider(), usePlatformServices() (+15 more)
...
Cohesion: 0.07
Nodes (19): 현재 범위: docs/development-update-2026-09-14.md, 2026-09-14 개발 현황 공유 및 화면 캡처, 정산 인원 및 비용 관리 화면, 해외여행 생성 및 공유 참여 폼, Trip Split 온보딩 및 홈 허브, 출발 전 준비 허브 (장소 후보 및 예약), 장소 보관함 및 키워드 검색, 장소 수동 입력 및 편집 폼 (+11 more)
...
Cohesion: 0.10
Nodes (23): PlatformServices, block(), edit(), renderWorksheet(), createInMemoryTripRepositories(), createdAt, expenses, GANGNEUNG_TRIP_ID (+15 more)

### Community 34 - "React 메모리 저장소 구현"
Cohesion: 0.29
Nodes (6): clone(), InMemoryTripRepositories, notFound(), sortById(), TripRepositorySeed, EntityId

### Community 35 - "React 여행 기능 화면"
Cohesion: 0.14
Nodes (23): useTripContext(), buildTransitUrl(), dateFormatter, enumerateDates(), formatDate(), ItineraryPage(), domesticChecklist, domesticReservations (+15 more)
...
Cohesion: 0.13
Nodes (10): call(), CreateTripCommand, CreateTripResult, FirebaseTripSessionService, invalidArgument(), JoinTripResult, MockTripSessionService, parseLocalDate() (+2 more)
...
Cohesion: 0.11
Nodes (11): 정산·영수증 기능 논의 (ST/OCR 기능 ID, 단일 원장), TASK 이슈 템플릿 (TASK-01~TASK-09), TASK-01 프로젝트 기반 및 앱 셸 구축, 현재 범위: MarkDown/task/task_function2_trip_share.md, 현재 범위: MarkDown/task/task_function6_settlement.md, 현재 범위: MarkDown/task/task_function7_ocr.md, TASK-07 영수증 OCR 및 번역 초안 검토, 현재 범위: MarkDown/task/task_function8_backup_export.md (+3 more)
...
Cohesion: 0.17
Nodes (14): categories, dateFormatter, Editor, formatDate(), formatHour(), ItineraryWorksheet(), ItineraryWorksheetProps, ScheduleDraft (+6 more)
...
Cohesion: 0.13
Nodes (9): EMPTY_DATA, TripContext, TripContextValue, TripDataState, PlannerMapPreviewProps, ItineraryRepository, PlacesRepository, ItineraryItem (+1 more)
...
Cohesion: 0.12
Nodes (15): dependencies, firebase-admin, firebase-functions, engines, node, firebase, firebase-tools, @types/node (+7 more)

### Community 69 - "IMB-03 참조 Emulator 회귀"
Cohesion: 0.19
Nodes (12): apps, audit(), checklist(), client(), create(), expectIntegrity(), itinerary(), reservation() (+4 more)
...
Cohesion: 0.15
Nodes (10): 도쿄 2025 여행 시트 탭1 고정 테스트 샘플, 현재 범위: docs/project-scope-review.md, Trip Split Flutter 전환 범위 점검 (코드 vs 방향 vs 회의 항목), 현재 범위: docs/sheet-export.md, Google Sheets 여행 보고서 내보내기 구현 및 권한 분리, 현재 범위: MarkDown/decision_history.md, 제품 기술 발전 이력 (강릉 HTML → React/PWA → Flutter Android), 일정·지도·준비·정산 통합 여행 세션 제품 정의 (+2 more)
...
Cohesion: 0.25
Nodes (8): Bundle, ByteArray, FlutterActivity, FlutterEngine, MainActivity, Intent, MethodChannel, Uri?
...
Cohesion: 0.20
Nodes (10): createMapRenderModel(), MapAdapter, MapPin, MapPolyline, MapRenderModel, buildGoogleMapsUrl(), clamp(), PlannerMapPreview() (+2 more)
...
Cohesion: 0.21
Nodes (9): 현재 범위: backend/README.md, 백엔드 Callable Functions 및 에뮬레이터 가이드, 일정·지도 백엔드 SPEC, Google Places Emulator·운영 미연결 경계, Firebase 연결 감사 (13개 Callable, Firestore 컬렉션 및 쓰기 제약), 현재 범위: docs/firebase-api-contract.md, Flutter 화면 제작 순서 및 13개 Callable 요청/응답 Wire 규격, 현재 범위: docs/frontend-api-handoff.md (+1 more)
...
Cohesion: 0.22
Nodes (8): CODE_MAP, MESSAGES, normalizeCode(), AppError, AppErrorCode, EpochMillis, epochMillisFromDate(), isEpochMillis()

### Community 88 - "Emulator 인증 도메인 테스트"
Cohesion: 0.17
Nodes (6): apps, apps, CreateTripResult, JoinTripResult, TestClient, @firebase/rules-unit-testing

### Community 89 - "일정·지도 실행과 검증"
Cohesion: 0.17
Nodes (12): 일정·지도 백엔드 실행 계획, IMB 작업별 검증·정지 계약, 일정·지도 백엔드 작업 목록, IMB-00 — 담당 하네스 작성 — 완료, IMB-01 — 실행 환경과 기준선 — 완료, IMB-02 — 기존 검색·링크 경계 회귀 — 완료, IMB-04 — Google 연결·링크 정책 확정 — 미착수, IMB-05 — 실제 장소 검색 경로 구현 — 미착수 (+4 more)
...
Cohesion: 0.18
Nodes (11): Firestore itinerary, 일정 placeId 참조 무결성 및 버전 강제 검증, Firestore places, 장소 클라이언트 직접 삭제 차단 (deletePlace 강제), createItineraryItem, createPlace, reorderItineraryItems, updateItineraryItem (+3 more)

### Community 96 - "Android 출시 검증 문서"
Cohesion: 0.22
Nodes (7): 현재 범위: docs/android-release-checklist.md, Android 내부 테스트·출시 점검 항목, 현재 범위: docs/development-kickoff.md, 2026-09-14 3인 개발 인계 (Callable 13개 구현, 외부 연동 미완료), Google Maps 어댑터·시트 내보내기·동기화 통합 기록, 현재 범위: MarkDown/task/task_function9_polish_release.md, TASK-09 Android 접근성, 권한 및 마감 품질
...
Cohesion: 0.24
Nodes (7): Firestore reservations, apps, client(), create(), setup(), saveReservation, watchReservations
...
Cohesion: 0.22
Nodes (5): 아키텍처 경계 및 불변 ID 규칙, 3인 역할 분담 규칙, Flutter 정적 분석 린트 설정, Flutter Android 앱 패키지 및 의존성 매니페스트, Flutter-Backend 모노레포 구조 및 3인 역할 경계

### Community 106 - "체크리스트 담당자 Rules"
Cohesion: 0.25
Nodes (9): Firestore checklistitems, Firestore expenses, Firestore 여행 멤버 검증, Firestore participants, Firestore Rules 저장 권한, IMB-03 — 일정·장소·준비 참조와 동시 변경 — Flutter 검증 대기, saveChecklist, setChecklistCompleted (+1 more)

### Community 107 - "Flutter 전환 범위 문서"
Cohesion: 0.29
Nodes (5): 현재 범위: docs/flutter-android-migration.md, Flutter Android 전환 계획 (Phase A~D), 현재 범위: docs/platform-handoff.md, 플랫폼·통합 기반 및 세션 주입 구조, 2026-08-28 Flutter Android 전환 결정 및 Tokyo JPY 우선
...
Cohesion: 0.25
Nodes (4): NavigationIconProps, navigationItems, TripNavigation(), TripNavigationProps
...
Cohesion: 0.38
Nodes (4): 일정·지도 기능 후보와 IT/PREP 기능 ID, 현재 범위: MarkDown/task/task_function4_itinerary.md, TASK-04 날짜별 일정 시간표, 순서 및 장소 연결, TASK-05 지도 핀, 동선 및 지도 adapter
...
- **Why does `플랫폼·통합 기반 및 세션 주입 구조` connect `Flutter 전환 범위 문서` to `Flutter 인증 진입 게이트`, `지출 상세 조회 화면`, `Flutter 여행 구독 세션`?**
...
- **Should `Flutter 공통 데이터 모델` be split into smaller, more focused modules?**
  _Cohesion score 0.023809523809523808 - nodes in this community are weakly interconnected._
- **Should `Flutter repository 입력 계약` be split into smaller, more focused modules?**

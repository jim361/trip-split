...
단계: A는 익명 세션·생성/참여 기반, B(P0)는 공유·설정·기본 내 여행 목록·본인 참여자 연결의 통합 검증이다. C(P1)에서 선택적 Google 계정 연결/접근 복구를 실기기로 확인하며 App Links 초대는 D의 별도 후속 범위다. 기존 Google 로그인 service 구현은 유지한다.
...
- [x] 공유 코드 복사와 Android share sheet 구현 (실기기 앱별 동작 QA는 별도)
- [ ] Android App Links 기반 초대 링크는 공유 코드 흐름이 안정된 뒤 별도 단계로 구현
- [x] 공유 코드로 여행 참여 구현
- [ ] 초대 링크 접속 시 여행 참여 구현 (D 후속, P0 완료 조건 아님)
- [x] 여행 기본 정보 수정 구현 (제목·기간)
- [x] 여행 데이터 실시간 구독 구현
...
- 공유 코드로 같은 여행 세션에 들어갈 수 있다. App Links 초대는 후속이며 P0 완료를 막지 않는다.

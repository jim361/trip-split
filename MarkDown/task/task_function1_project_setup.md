...
- [ ] 기존 React 목업의 보존 범위를 기록한 뒤 `frontend/`에 Flutter 프로젝트 생성
- [ ] Android를 첫 실행 대상으로 설정하고 `minSdk 24`, `targetSdk 36` 검증
- [ ] Git 기본 브랜치와 작업 규칙 정리
- [ ] `flutter_lints`와 `analysis_options.yaml` 설정
- [ ] Dart format check, unit/widget test와 integration test 기반 설정
- [ ] Flutter SDK와 Dart dependency 버전 고정
...
- [ ] Flutter router에 일정·지도, 준비, 비용 route와 receipts 하위 route 구성
- [ ] 기존 `/trips/:tripId/map` deep link를 확대 일정·지도 상태로 연결할 호환 규칙 정의
- [ ] Android `NavigationBar`, `SafeArea`, system back과 키보드 inset을 처리하는 `TripShell` 구현
...
- [ ] 고정 ID `tokyo-2026-11` fixture 추가, 기존 강릉 fixture는 KRW 회귀용으로 보존
- [ ] Material Theme와 디자인 토큰 초안 작성
- [ ] backend Node.js 22·TypeScript와 Firestore Rules/Emulator 구조 보존
- [ ] Flutter Web은 target 파일만 유지할 수 있으나 첫 완료 게이트와 배포에서는 제외

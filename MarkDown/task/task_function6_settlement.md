...
> **[TASK-06 · 정산]** 지출 원장, 배분과 참여자별 정산 기능입니다.
...
- 선행: Task 1의 Flutter·Dart 테스트 기반, Firebase Emulator와 Auth/`TripSession`이 준비되어야 한다.
- 선행 계약: `TripMember`와 `Participant` 분리, 공통 ID·timestamp·오류 형식, repository 인터페이스와 `tokyo-2026-11` fixture가 확정되어야 한다.
...
```ts
// Firestore wire 계약 표기다. Flutter 엔진은 같은 불변식을 Dart 모델로 구현한다.
type AllocationMethod = "equal" | "itemized" | "custom";

type MoneyAllocation = {
  participantId: string;
  amount: number; // ISO 통화의 최소 단위 정수
};
...
서버 handler와 Flutter adapter는 다음 wire를 함께 구현한다. 아래 wire는 2026-09-14 서버 handler·validator와 Flutter adapter에 구현했다. P1 itemized까지 확장했으며 실제 외부 OCR는 별도다.
...
- 균등 분할은 0 방향 정수 나눗셈으로 계산하고 남은 최소 단위 ±1을 UI에 표시된 `consumers` 순서대로 배분한다. 같은 입력은 모든 클라이언트에서 같은 결과를 만든다.
- 참여자 `p`의 파생값은 다음과 같다.
...
- [x] Flutter에서 참여자별 부담 금액을 직접 입력하고 합계를 검증하는 `custom` 배분을 구현한다.
...
- [x] Flutter에서 수동 지출의 제목, 날짜, 카테고리, 총액, 결제자, 장소·일정 연결, 메모 입력을 구현한다.
...
- [x] Flutter mock에서 저장 중 중복 제출을 막고 성공 후 저장된 지출 상세로 이동한다.
...
- [ ] Android 비용 화면은 `개인 요약 → 최종 정산 → 지출 목록` 순서로 배치한다.
...
- [ ] 로딩·빈 상태·구독 오류·권한 오류를 구현하고 금액은 locale 형식과 ISO 통화 코드를 함께 표시한다.

완료 조건:

- 한 사용자가 참여자를 바꿔 보아도 결제액·부담액·net의 의미가 섞이지 않는다.
- Android handset에서 카테고리 합계, 상세 내역, 송금 제안과 지출 CRUD에 접근할 수 있다.

## 단위 테스트

- [x] `10,000원 / 3명`은 소비자 순서대로 `3,334 / 3,333 / 3,333원`이 된다.
- [x] `1원 / 3명`은 `1 / 0 / 0원`이 되며 세 consumer의 배분 행이 모두 유지된다.
- [x] `-1,000원 할인 / 3명`은 소비자 순서대로 `-334 / -333 / -333원`이 된다.
...
- [ ] Flutter widget/integration test에서 요약→최종 정산→지출 목록 순서와 키보드·뒤로 가기 동작을 확인한다.
...
- `ExpenseDraft.validateManual`을 Flutter 폼과 mock 저장 경계에서 재사용한다. 실제 날짜, KRW/JPY 안전한 정수 금액, 결제액·배분 합계, 소비자 집합·중복, 균등 나머지, 활성 참여자·기존 비활성 참여자와 같은 여행의 장소·일정 참조를 확인한다. 이 메서드는 backend TypeScript validator의 구현 완료를 뜻하지 않는다.
- 결제자 선택에는 Participant ID를 사용한다. 로그인 UID와 연결된 활성 참여자가 한 명일 때만 초기 결제자로 선택한다. 균등 나머지는 화면에 표시된 소비자 순서를 따른다.
- 수정은 생성 감사 필드를 보존하고 삭제는 없는 ID에도 성공한다. 저장 후 기존 Stream으로 통화별 전체 금액과 기준 통화의 개인 요약을 갱신한다. 전체 통화별 개인 집계·송금 제안과 정산 문구 복사는 후속이다.
- 저장 중 중복 제출·뒤로 가기를 차단하고 오류 후 입력을 유지한다. 신규 저장의 통신 결과가 불명확하면 재저장을 막고 현재 원장에서 중복 여부를 확인한 뒤 사용자가 재시도를 허용한다. exactly-once 보장은 아니다.
- `TripSessionController`는 여섯 구독의 첫 응답을 모두 받기 전까지 로딩 상태를 유지해 아직 도착하지 않은 참여자·지출을 빈 목록으로 확정하지 않는다.
- 실제 Firestore 지출 쓰기는 기존 Rules에서 차단된다. backend 지출 Callable 및 Flutter 쓰기 adapter 연결은 미완료이며 이번 변경은 mock 흐름이다.
- 검증: `npm run verify:fast` 통과(React 59·backend 27), `npm run verify:flutter:full` 통과(Flutter 108·분석·포맷·Android debug APK). 금액·참여자 저장 경계, CRUD·요약 갱신, 응답 유실·중복 재시도, 큰 글씨·키보드·미저장 입력과 세션 초기 로딩을 포함한다. 로컬 구현이며 commit/push는 별도다.
...
## 2026-09-14 정산·서버 확장 인계

equal/custom/itemized ExpenseDraft 및 TypeScript validator, 세 지출 Callable과 Flutter adapter를 구현했다. 개인 화면은 통화별 결제/부담/net, 날짜·유형 필터, 장소·항목별 개인 부담, 결정적인 송금 제안·전체 통화 공유 문구를 제공한다. 참여자 계정 연결과 비활성화는 기존 원장을 유지한다. Rules의 expense 직접 쓰기 금지는 유지한다. 신규 생성의 결과가 불확실하면 원장을 확인한 뒤 재시도하며 서버 멱등 생성 키는 아직 없다.

...
이는 사람이 읽는 보고서다. 아래 `.trip.json` 백업·복원 스키마와 기존 작업 ID는 유지한다. 실제 생성 구현은 아직 없으며 [시트 내보내기 인계](../../docs/sheet-export.md)에 화면 흐름·연결 경계·완료 기준을 둔다.

- [x] 첫 번째 탭에서 고정 일정·지출 샘플 추출 및 원본/검산값 보존
- [x] Flutter 중심 출력과 기존 repository·Dart 정산 재사용 방향 정리
- [ ] 날짜·일정·지출 개수에 맞춘 보고서 배치·서식 생성
- [ ] 내보내기 옵션·미리보기·저장·실패 상태
- [ ] 필요한 여행 데이터 전체 조회 및 통화별 합계·정산 연결
- [ ] Google OAuth 추가 권한과 Sheets 생성 service

...
- 장소 보관함과 지역별 장소 provider/source(국내 NAVER, 해외 Google)는 일정 및 지도 입력의 일부이므로 장소·일정·지도 담당이 소유한다.
...
- 지원 통화는 KRW와 JPY이며 각 통화 금액은 최소 단위 정수다. 균등 분할의 잔액은 화면에 표시된 소비자 순서대로 배분한다.
- 장소 API 응답은 앱 내부 `Place`로 정규화한다. 지도는 `Place` 좌표와 `ItineraryItem.order`만 입력받고 실제 도로 경로를 계산하지 않는다.
- 여행 내부 주요 내비게이션은 `일정·지도`, `준비`, `비용` 세 개와 동일한 순서로 고정한다. 영수증/OCR은 비용 화면의 하위 진입점이며, 지도는 일정 상단에서 확대할 수 있고 장소 보관함은 통합 화면 안의 패널 또는 바텀시트다.
...
- 고정 ID 강릉·도쿄 fixture를 화면, 순수 함수 테스트, repository 통합 테스트에 사용한다. fixture 변경은 공통 계약 변경으로 취급한다.
...
3. `TASK-03` — [task_function3_places.md](task_function3_places.md)
   - 장소 보관함, 국내 NAVER·해외 Google provider/source, 장소 링크 붙여넣기, 직접 입력, `Place` 정규화
...
5. `TASK-05` — [task_function5_map.md](task_function5_map.md)
   - 국내 NAVER 지도와 해외 Google Maps 스타일 mock, 일정 순서 기반 번호 핀, 날짜별 색상과 직선 동선
...
7. `TASK-07` — [task_function7_ocr.md](task_function7_ocr.md)
   - 비용 화면 하위 영수증/OCR 진입, 로컬 이미지 선택, stateless CLOVA OCR, 수정 가능한 초안, 항목 분할, 사용자 확정 저장
...
- 공통 타입, Firestore 경로, repository 인터페이스, 오류 형식, 고정 ID 강릉·도쿄 fixture를 먼저 확정한다.
...
- 장소·일정·지도: 지역별 place provider mock → 일정 UI → 지도 표시 → 국내 NAVER API 연결 순서로 구현하고, 해외 Google은 스타일 mock·외부 링크로 유지한다.
- 플랫폼·통합: 여행 생성·참여·공유 → 실시간 세션 → PWA → 데이터 모델 안정화 후 `.trip.json` 순서로 구현한다.
- 외부 API는 mock 흐름과 실패 상태가 완성된 뒤 연결한다.

### 단계 D — 통합 체크포인트

1. mock 데이터로 세 개 주요 화면과 비용 하위 영수증/OCR 진입, 핵심 CRUD, 로딩·빈 상태·오류 상태를 확인한다.
2. Firebase Emulator에서 익명 사용자 두 명이 같은 여행에 들어와 장소·일정·지출 변경을 실시간으로 확인한다.
3. 국내 NAVER 장소·지도 API와 CLOVA OCR을 실제 환경에서 확인하고, 해외 Google은 지도 카드·외부 링크 mock만 검증한다. 이번 단계에는 Google Maps SDK, Places API, Routes API를 연결하지 않으며 외부 API 실패가 수동 입력 흐름을 막지 않는지 검증한다.
4. 390px 모바일 PWA와 PC에서 동일한 세 개 내비게이션(`일정·지도 / 준비 / 비용`), 비용 하위 영수증/OCR 진입, 지정된 화면 순서, 백업·복원, 전체 회귀 테스트를 확인한다.
...
- 390px 모바일과 PC에서 `일정·지도 / 준비 / 비용`의 동일한 정보 구조와 비용 하위 영수증/OCR 진입, 키보드·터치 조작을 확인한다.

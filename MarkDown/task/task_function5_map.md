...
사용자(플랫폼·통합)는 Flutter 지도 SDK adapter·핀·동선·확대 UI·외부 길찾기와 Dart 테스트를 맡는다. 일정·지도 백엔드 담당은 TASK-03의 장소 검색·좌표 정규화와 데이터 검증을 제공한다. Google 서버 키는 backend에, Android 지도 키는 제한된 앱 설정 경계에 둔다.
...
mock 번호 핀 선택은 해당 일정 편집으로, 지도 열기는 Android 외부 Google Maps 장소/구간 URL로 연결했다. 날짜·A/B 순서와 지도 표시 모델을 공유한다. 실제 Google 지도 SDK·지도 좌표 기반 렌더링·유료 Routes API는 아직 연결하지 않았다.

...
국내 NAVER 지도와 해외 Google Maps 스타일 mock 위에 타임테이블 기반 번호 핀과 날짜별 직선 동선을 표시한다.
...
- [ ] 강릉·도쿄 fixture로 핀 번호, 날짜별 색상과 직선 segment를 만드는 순수 변환 함수와 단위 테스트 구현
- [ ] mock map adapter로 로딩·빈 상태·좌표 누락·일정 재정렬 화면 검증
- [ ] 국내 NAVER 지도 SDK는 공통 script loader로 한 번만 로드하고, 해외 Google 지도는 SDK 없이 mock 카드·외부 링크로 표시
- [ ] 지도 컨테이너 컴포넌트 구현

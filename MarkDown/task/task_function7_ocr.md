...
정산·영수증 담당이 `parseReceipt`, OCR·번역 검토 화면과 지출 변환을 소유한다. 플랫폼·통합 담당은 Functions 공통 진입점, Auth/`TripSession`, 환경변수 예시와 보안 규칙 변경을 검토·병합한다. 장소·일정·지도 담당은 장소 선택 Widget의 공개 인터페이스만 제공한다.

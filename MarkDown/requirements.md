...
P0는 Flutter 기반·여행 세션·공유/설정·기본 내 여행 목록·Google 일정/지도·최소 준비·수동 equal/custom 정산이고, P1은 itemized 정산·영수증 OCR/번역·선택적 Google 계정 연결/복구 검증과 Android 배포 품질이다. 기본 내 여행 목록은 2026-09-14 화면 결정과 구현에 맞춰 P0로 정리한다. P0를 통과하기 전에 P1 외부 API를 연결하지 않는다. 이미 구현한 P1 화면·mock·validator와 테스트는 유지하되 P1 실제 연결 완료로 표시하지 않는다.

실행 순서는 Phase A(기반) → Phase B(P0 통합) → Phase C(P1·출시 품질) → Phase D(P2/별도 채택 후보)다. 실제 Google 지도·장소 검색은 B이며 Routes API·이동 시간 계산은 D다. `.trip.json`은 핵심 데이터 안정화 뒤 B 마감에서 실제 여행 전 백업/복원을 검증한다. Google Sheets 양식·미리보기는 사용자의 별도 병행 작업이고 B/C 출시 통과 조건은 아니다. 담당별 착수와 완료 조건은 [개발 시작 안내](../docs/development-kickoff.md)를 따른다. 실제 Firebase·유료 API·배포의 실행 승인은 개발 단계와 구분한다.
...
### 추가 개발 — Google Sheets 여행 보고서 (사용자 병행 작업)
...
- 고정 샘플 양식·미리보기는 P0와 병행 가능하다. Sheets/CSV를 앱으로 가져오는 기능은 별도 후속 후보이며, 보고서 출력이 `.trip.json` 백업/복원을 대신하지 않는다.
...
- equal/custom 수동 지출의 생성·수정·삭제, 권한·참여자·금액 검증과 두 Android 클라이언트 실시간 반영을 확인한다.
- 균등 나머지·직접 지정 합계가 총액과 일치하고 통화별 결제액/부담액/net·송금 문구가 같은 원장에서 나온다.
- 과거 비활성 참여자 참조를 보존하고 신규 선택·잘못된 참조를 거부한다. 응답 유실 시 자동 재생성하지 않으며 저장 실패·재접속을 복구할 수 있다.

### Phase C · P1 항목별 정산·OCR

아래 항목형·OCR 조건은 P1 완료 조건이며 P0 외부 연결 착수의 선행 조건으로 소급하지 않는다. 기존 자동 테스트는 P1 회귀까지 그대로 실행한다.

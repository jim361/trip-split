...
### 추가 개발 — Google Sheets 여행 보고서

- 일정·지출·통화별 정산을 참고 양식에 맞춰 새 Google Sheet로 내보낸다. 현재는 샘플과 구현 방향만 준비됐으며 첫 Android 출시 완료 여부와 별도로 추적한다.
- 날짜 칸과 지출 행은 여행 기간·항목 수에 맞춰 확장하며, 시간·장소가 없는 일정과 일정에 연결되지 않은 지출도 포함한다.
- 기존 repository와 Dart 정산을 재사용해 Flutter service에서 처리한다. 사용자 OAuth 권한으로 생성하며 시트 전용 신규 서버 API는 요구하지 않는다.
- 원본 Google Sheet·여행 원장은 변경하지 않는다. 기존 `.trip.json` 백업/복원 형식과 구분하고, 자동 환산·방문 이력 추정·양방향 동기화는 추가하지 않는다.
- 상세 흐름과 검증은 [시트 내보내기](../docs/sheet-export.md), 진행 상태는 TASK-08을 따른다.

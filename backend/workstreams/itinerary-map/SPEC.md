...
> 담당: 일정·지도 백엔드. 갱신일: 2026-09-15. 확인 기준: `dev` / `e6e706191c82fc6027331a958a50aff165de9874` 위 IMB-03 미커밋 변경.
...
IMB-03의 사용자 후속 진행 지시는 필요한 Rules·공통 삭제·정산 참조 쓰기·Flutter repository·계약·검증 변경을 허용한 예외다. Astra가 통합 검토하며 원격 반영·배포 승인과 구분한다.
...
표준 HTTPS 포트 `:443`은 Node `URL`이 기본 포트로 정규화하므로 위 지원 URL에서 허용한다. `:80`, `:444`, `:8080` 같은 비표준 HTTPS 포트는 거부한다. IMB-02에서 기존 동작을 회귀로 확인하며 링크 지원 범위를 확대하지 않는다.
...
IMB-03 후속 구현은 같은 여행 장소/일정 존재를 `existsAfter()`로 확인한다. 일정·예약 참조 변경과 연결된 예약 삭제는 여행 `referenceVersion`을 같은 transaction에서 +1 갱신한다. 장소·일정 삭제는 공통 Callable만 허용하며 일정/예약/지출에서 참조 중이면 거부한다. 기존 고아 참조는 자동 삭제하지 않고 명시적 재선택/해제로 복구한다. 구현·검증 상태는 tasks의 최신 기록을 따른다.

- 장소 없는 일정과 정상 연결·해제를 허용한다. 없는 장소·다른 여행 장소의 연결은 저장 경계에서 거부하고 기존 문서가 변하지 않는지 검사한다.
...
- 사용자 후속 진행 지시에 따라 **참조 중 삭제 거부**를 적용한다. 화면 안내와 서버를 일치시키며 사용자에게 연결 해제를 요구한다. 삭제 Callable은 여행 버전을 읽은 transaction에서 역참조를 조회하고 버전 증가와 삭제를 함께 수행한다. 정산 저장/수정/삭제도 같은 여행 버전을 증가시킨다. `existsAfter()`만으로 후속 삭제 문제를 해결한다고 판단하지 않는다.
- 여행별 공통 버전은 기존 문서 생략=0이며 실제 경합이 커질 때만 대상별 보호 단위로 분리한다. 새 모델·역참조 컬렉션·자동 cascade는 추가하지 않는다. 후속 공통 변경 승인 해석·실제 검증·미반영 상태는 tasks에 명시하고 commit·push·배포 승인과 구분한다.

## 6. 제외 범위와 완료 조건

이번 IMB-03 예외로 Flutter repository의 참조 경계만 변경한다. Flutter 화면·지도 SDK·시트용 서버 함수, OCR·번역·정산 알고리즘, NAVER, Routes API·자동 이동 시간, Gemini, 백업 서버, 운영 배포는 추가하지 않는다. 기존 P1 구현과 회귀는 유지한다.

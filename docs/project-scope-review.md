...
전환 당시 점검 기준일은 2026-08-28이며 실행 지시는 2026-09-14에 보정했다. Flutter Android 우선, 도쿄·Google 우선과 Web 후속 결정은 `MarkDown/` 계약에 반영했다. 현재는 **Phase B(P0) 통합**이며 담당별 실제 착수는 [개발 시작 안내](development-kickoff.md), 구현 상태는 [화면/API 인계](frontend-api-handoff.md)를 따른다. 아래 전환 당시 상태·질문은 역사 기록이며 현재의 미구현 목록으로 사용하지 않는다. 계약과 충돌하면 `MarkDown/`이 우선한다.
...
`frontend/`의 제품 앱은 Flutter Android이며 주요 일정·준비·수동 정산·공유 화면과 repository를 구현했다. `backend/`는 Node.js·TypeScript Firebase 기반으로 11개 Callable과 Emulator 검증이 있다. P1 itemized·영수증 검토·OCR mock도 선행 구현됐지만 외부 OCR는 미연결이다. React/Vite는 GitHub Pages mock으로 보존한다. 두 Android 클라이언트의 전체 도메인 QA와 실제 지도/검색·백업은 아직 남아 있다.
...
현재 B에서는 Google Maps URL 지원 범위·실제 연결과 동시 변경/재접속·두 기기 QA를 확인한다. 최소 준비와 통화별 개인 정산은 이미 구현돼 있으므로 다시 설계할 첫 작업으로 배정하지 않는다. OCR benchmark 합격 기준은 B 완료 뒤 C의 작업이다. Flutter 선택, Google 우선과 지출별 통화 허용은 다시 논의할 열린 항목이 아니다.

## 2. 전환 당시 구현 상태 — 2026-08-28 기록

| 영역          | 지금 확인할 수 있는 것                                                                                                 | 아직 구현되지 않았거나 확인할 것                                                                              |
| ------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 프로젝트 기반 | Flutter Android scaffold·세 탭 mock·Dart 검증과 CI debug APK, 공통 Callable Auth·멤버·오류 wire, 임시 React Pages 목업 | Android 실기기 QA와 Flutter client→Firebase Emulator 수직 검증                                                |
...
| 영수증        | bytes 기반 `parseReceipt` 요청·mock 응답 경계, appCode 오류 mapping과 검토 placeholder                                 | 이미지 선택, OCR Function/provider, 수정·배분·확정 저장                                                       |
...
위 표는 전환 당시의 기록이다. 이후 준비 저장·정산·공유와 영수증 검토 기반은 구현됐다. 최신 완료/미완료는 [화면/API 인계](frontend-api-handoff.md)를 따른다. GitHub Pages는 React 참고 목업이며 최신 Flutter 화면이 자동 반영되는 배포본이 아니다.

## 3. 전환 당시 데이터 경계와 검토 질문

여기서 제기한 본인 참여자 연결·수동 순서·통화별 개인 정산은 이후 구현됐다. 당시 질문 전체를 현재 미확정 계약으로 취급하지 않고 현재 wire는 [Firebase/API 계약](firebase-api-contract.md)을 확인한다.
...
| 비용 원장        | 한 `Expense`에서 결제자와 소비자를 표현하고 import·Callable이 같은 runtime validator를 쓰도록 계약돼 있다. | validator 구현과 필드별 오류·비활성 Participant를 Android 입력 화면에서 어떻게 보여줄지 |
| 통화             | `defaultCurrency`는 입력 기본값이고 지출별 ISO 통화를 허용하며 환율 없이 통화별로 계산한다.                | 첫 Android UI에서 통화 선택과 통화별 결과를 얼마나 간결하게 보여줄지                    |
...
- 날짜별 장소·일정을 추가·수정·삭제하고 길게 눌러 끌어 순서를 바꾼다. 사용자의 2026-09-14 요청대로 위·아래 이동 버튼을 완료 기준으로 대체하지 않는다. 시간순 정렬은 별도 확인하는 보조 action이다.
...
- 결제액·부담액·net과 개인 소비 내역을 결정적으로 계산하고 모든 결과와 송금 문구를 통화별로 분리한다.
- 솔로 여행에서는 송금 영역을 숨기고 카테고리·날짜별 개인 소비를 강조한다.

#### 최소 준비·목록

- 예약 제목·유형·상태·URL·메모와 공동/개인 체크리스트의 저장·완료·두 기기 반영을 확인한다. `personal`은 멤버 공유 분류다.
- 기본 내 여행 목록·공유/설정은 P0이며 구현된 `listMyTrips`를 재사용한다. Google 계정 연결/복구의 실기기 검증은 P1이다.
...
- 최소 준비는 P0다. 알림·첨부·민감 문서 보관은 P1에 자동 추가하지 않는다.
...
- 선택적 Google 계정 연결 시 UID 유지·여행 접근 복구와 익명 세션 소실 안내를 실기기로 검증한다. 기본 목록을 다시 만드는 단계가 아니다.
- Flutter Web·iOS·NAVER·실제 경로 계산은 P1이 아닌 Phase D 후속이다.

## 5. 추가로 검토할 기능 후보

이 표는 상세 문서의 기존 기능 ID를 요약한다. 새 기능 ID는 해당 영역 문서의 `추가 제안` 표에서만 배정한다. Sheets/CSV 가져오기는 현재 사용자 병행 작업인 [Google Sheets 보고서 출력](sheet-export.md)과 별개이며 지금 구현할 기능으로 채택된 것이 아니다.
...
| 개인/공동 체크리스트와 담당자       | `PREP-02`        | 공동 작업과 개인 준비물을 구분한다. 마감·알림·첨부는 넣지 않는다.                                 | P0 구현·QA 대상 |
...
## 7. 과거 제품 질문과 현재 확인 단계

Flutter·Android·Google 우선과 구현 순서는 확정됐다. 아래는 당시 질문 목록이다. 본인 Participant 연결·준비 wire·통화별 표시·비활성 이력 정책은 이후 구현돼 현재는 회귀/실기기 확인 대상이다. URL 지원 범위는 B에서, OCR 기준·계정 복구는 C에서 확인한다.
...
| P0       | 통화별 결과를 Android에서 어떻게 보여줄까?        | 지출별 통화를 허용하고 모든 합계·net·송금안을 통화별 섹션으로 분리한다.                  |
...
| P0       | Firestore offline write를 UI에서 어떻게 표시할까? | 로컬 반영, 동기화 대기, 서버 완료와 실패를 구분한다. Callable 생성은 별도 온라인 경계다. |
...
- KRW와 JPY의 합계·net·송금 문구가 서로 합산되지 않는다.
...
- [x] 핵심 Flutter Dart 모델과 mock/Firestore repository 계약을 구현한다. 준비 모델·repository·Rules도 2026-09-14 구현했으며 두 기기 QA는 남아 있다.
...
| 담당                | 첫 구현 묶음                                                                                      | 통합 전 확인                                                                 |
| ------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| 플랫폼·통합(사용자) | 기존 P0 Flutter/Emulator 통합·두 기기 QA, 실제 지도 SDK, 핵심 안정화 뒤 백업. 시트 양식 병행 가능 | Dart 계산·repository·공통 모델·Rules와 Android 상태/오류                     |
| 일정·지도 백엔드    | 기존 검색/링크 검토, 일정·장소·준비 동시 변경 검증, 승인된 Google Places 연결                     | URL·좌표·권한·실패/timeout·참조. Flutter SDK는 사용자                        |
| 정산 백엔드         | 기존 equal/custom CRUD·금액·권한·참조·응답 유실 회귀, 두 기기 정산 검증                           | Dart 계산과 서버 fixture의 통화별 일치. itemized/OCR 신규 연결은 B 완료 뒤 C |
